...
class ContractService implements IContractService {
  ...
  ContractFunction _viewSaleFunction() => contract.function('viewSale');
  ContractFunction _sendFunction() => contract.function('transfer');
  ContractFunction _privateSaleFunction() => contract.function('privateSale');
  ...
  Future<String?> send(
    ...
    try {
      final transation = Transaction.callContract(
        contract: contract,
  ...
  Future<double> getTokenRate() async {
    final response = await client.call(
      contract: contract,
      function: _viewSaleFunction(),
      params: [],
    );
    var rate = response.last as BigInt;
    return rate.toDouble() / pow(10, Constants.DECIMALS);
  }
